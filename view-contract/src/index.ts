...
function renderTemplate(token: Token, template: string): string {
  ...
  // NEP-177 token metadata replacements
  {
    ...
    rendered = rendered.replaceAll(
      '{token.media_hash}',
      token.metadata.media_hash,
    );
    rendered = rendered.replaceAll(
      '{token.copies}',
      token.metadata.copies.toString(),
    );
    rendered = rendered.replaceAll(
      '{token.issued_at}',
    ...
    rendered = rendered.replaceAll(
      '{token.starts_at}',
      token.metadata.starts_at,
    );
    rendered = rendered.replaceAll(
      '{token.updated_at}',
      token.metadata.updated_at,
    );
    rendered = rendered.replaceAll(
      '{token.reference}',
    ...
    rendered = rendered.replaceAll(
      '{token.reference_hash}',
      token.metadata.reference_hash,
    );
  }
  ...
  // Certification metadata replacements
  {
    ...
    rendered = rendered.replaceAll(
      '{cert.authority_name}',
      jsonValString(certification, 'authority_name'),
    );
    rendered = rendered.replaceAll(
      '{cert.authority_id}',
      jsonValString(certification, 'authority_id'),
    );
    rendered = rendered.replaceAll(
      '{cert.program}',
      jsonValString(certification, 'program'),
    );
    rendered = rendered.replaceAll(
      '{cert.program_name}',
      jsonValString(certification, 'program_name'),
    );
    rendered = rendered.replaceAll(
      '{cert.program_start_date}',
      jsonValString(certification, 'program_start_date'),
    );
    rendered = rendered.replaceAll(
      '{cert.program_end_date}',
      jsonValString(certification, 'program_end_date'),
    );
    rendered = rendered.replaceAll(
      '{cert.original_recipient_id}',
      jsonValString(certification, 'original_recipient_id'),
    );
    rendered = rendered.replaceAll(
      '{cert.original_recipient_name}',
      jsonValString(certification, 'original_recipient_name'),
    );
    rendered = rendered.replaceAll(
      '{cert.valid}',
      jsonValString(certification, 'valid'),
    );
    rendered = rendered.replaceAll(
      '{cert.memo}',
      jsonValString(certification, 'memo'),
    );
  }
