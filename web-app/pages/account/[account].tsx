...
function Tile({ tokenId }: { tokenId: string }): JSX.Element {
  ...
  return (
    <li className="object-cover shadow-lg rounded-lg">
      <div className="space-y-4" key={tokenId}>
        <div className="aspect-w-3 aspect-h-2">
          <a href={`/certificate/${tokenId}`}>
            <img className="object-cover shadow-sm rounded-lg" src={svgUrl} alt={`certificate ${tokenId}`} />
          </a>
        </div>

        <div className="space-y-2">
          <div className="text-lg leading-6 font-medium space-y-1">
            <p className="text-indigo-600">certificate name</p>
          </div>
          ...
          <div className="space-y-12">
            ...
            <div>
              {certificates.length > 0 ? (
                <ul className="grid sm:grid sm:grid-cols-2 sm:gap-x-6 sm:gap-y-12 sm:space-y-0 lg:grid-cols-3 lg:gap-x-8">
                  {certificates.map((tokenId: string) => (
