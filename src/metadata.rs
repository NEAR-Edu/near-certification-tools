...
mod tests {
    ...
    fn test_metadata() -> CertificationExtraMetadata {
        CertificationExtraMetadata {
            authority_account_id: Some("test_authority.near".parse().unwrap()),
    ...
    fn metadata_json_serialization_and_deserialization() {
        ...
        assert_eq!(deserialized.authority_account_id.as_ref().unwrap().to_string(), "test_authority.near");
        assert_eq!(deserialized.program_id.as_ref().unwrap().to_string(), "PRG101");
